...
class ExtendedBlockMatrix(blocks: RDD[((Int, Int), Matrix)],
	...
	/**
	 * Divide each column of the matrix by the corresponding lambda value.
	 */
	def divideByLambdas(lambdas: Array[Double]): ExtendedBlockMatrix = {
		new ExtendedBlockMatrix(
			blocks.map(b => {
				b._2 match {
					case _: DenseMatrix => {
						(b._1, Matrices.fromBreeze({
							val m = b._2.asBreeze.toDenseMatrix
							for (k <- lambdas.indices) {
								m(::, k) := m(::, k) *:* (1 / lambdas(k))
							}
							m
						}))
...
object ExtendedBlockMatrix {
	...
	/**
	 * Creates an [[ExtendedBlockMatrix]] with a HOSVD initialization.
	 */
	def hosvd(tensor: Tensor, rank: Int, dimension: Int)(implicit spark: SparkSession): ExtendedBlockMatrix = {
		ExtendedBlockMatrix.fromBreeze(tensor.matricization(dimension, true).computeSVD(rank).V.asBreeze)
	}
