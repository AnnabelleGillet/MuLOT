	...
	object Initializers {
		def gaussian(tensor: Tensor, rank: Int): Array[DenseMatrix[Double]] = {
			(for (i <- 1 until tensor.order) yield {
				var matrix = abs(DenseMatrix.rand(tensor.dimensionsSize(i), rank, breeze.stats.distributions.Gaussian(0.0, 1.0)))
				matrix /= max(matrix)
			}).toArray
		}
...
class ALS private(val tensor: Tensor, val rank: Int)
	extends mulot.core.tensordecomposition.cp.ALS[Tensor, DenseMatrix[Double], Map[String, Array[Map[Any, Double]]]]
		with Logging {
	
	override protected var initializer = Initializers.gaussian
	...
	override def execute(): Kruskal = {
		...
		while (!termination) {
			...
			for (i <- 0 until tensor.order) {
				...
				// MTTKRP
				val mttkrp = computeMTTKRP(tensorData, i, tensor.dimensionsSize(i), tensor.dimensionsSize, rank, factorMatrices)
			...
			if (nbIterations > 1) {
				val begin = System.currentTimeMillis()
				fms = factorMatchScore(factorMatrices, lambdas, lastIterationFactorMatrices, lastIterationLambdas)
	 ...
	 */
	private def computeMTTKRP(tensorData: Map[Array[Int], Double],
		...
		for ((keys, value) <- tensorData.par) {
			...
			// Find the corresponding value in the corresponding block of the matrices of the Khatri Rao product
			var vectorValue = DenseVector.fill[Double](rank, value).t
	 ...
	 */
	private def factorMatchScore(currentMatrices: Array[DenseMatrix[Double]], currentLambdas: Array[Double],
